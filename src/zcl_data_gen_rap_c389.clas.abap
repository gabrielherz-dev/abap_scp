CLASS zcl_data_gen_rap_c389 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_data_gen_rap_c389 IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.

    out->write( 'Adding Travel data' ).

    DELETE FROM ztravel_c389_a.

    INSERT ztravel_c389_a FROM (
    ...
    DELETE FROM zbooking_c389_a.

    INSERT zbooking_c389_a FROM (

        SELECT
          FROM /dmo/booking
          JOIN ztravel_c389_a ON /dmo/booking~travel_id = ztravel_c389_a~travel_id
          JOIN /dmo/travel ON /dmo/travel~travel_id = /dmo/booking~travel_id
          FIELDS  "client,
                  uuid( ) AS booking_uuid,
                  ztravel_c389_a~travel_uuid AS parent_uuid,
                  ...
                  ztravel_c389_a~last_changed_at AS local_last_changed_at ).


    DELETE FROM zbksppl_c389_a.

    out->write( 'Adding Booking Supplements data' ).

    INSERT zbksppl_c389_a FROM (
       SELECT FROM /dmo/book_suppl AS supp
              JOIN ztravel_c389_a  AS trvl ON trvl~travel_id = supp~travel_id
              JOIN zbooking_c389_a AS book ON book~parent_uuid = trvl~travel_uuid
